...
Language AUCs miss most of their bars at 170M, where instructions tokenize to only 16 to 30 tokens, while on the fine-tuned RDT-1B language deletion improves to passing and language insertion is seed-sensitive, passing at two of three evaluation seeds.
...
Ranking-based metrics, not magnitudes, are the right faithfulness tool.
...
When a robot fails to grasp an object or misreads an instruction, practitioners cannot tell whether the failure came from visual misattention, language mis-grounding, or proprioceptive misreading without inspecting model internals by hand.
...
Hooker et al.~\shortcite{hooker2019roar} evaluate attributions by retraining on ablated inputs, Rong et al.~\shortcite{rong2022road} show that deletion without retraining leaks information through the shape of the mask itself, and Tomsett et al.~\shortcite{tomsett2020sanity} find that common saliency metrics are themselves unreliable.
...
In continuous control, VisualBackProp~\cite{bojarski2018visualbackprop} visualizes CNN saliency for end-to-end driving policies.
...
\begin{equation}
\mathrm{IG}_i \approx \frac{x_i - x'_i}{m+1} \sum_{k=0}^{m} \frac{\partial F}{\partial x_i}\bigg|_{x' + \frac{k}{m}(x - x')} .
\label{eq:riemann}
\end{equation}
...
Here $D{=}512$ is the number of active action entries, namely the 64-step chunk over the eight active dimensions, so the target is proportional to the negative mean squared deviation per entry and its scale does not depend on chunk length.
...
\item \textbf{State.} Freeze vision and language and interpolate the 128-dimensional unified state vector (eight active dimensions for the Franka arm, namely seven joints plus the gripper) from zeros to its real value. The state-adaptor MLP sits inside the forward function to preserve per-joint structure.
\end{itemize}
...
The encoder output is zero-padded to the stored embedding length, and the language forward passes attend over the task's real token span, so the attended baseline is the encoded token pair followed by zero vectors at the remaining attended positions.
A zero vector is numerically safe through the GELU-based language adaptor but is semantically degenerate, and it roughly doubled the completeness error in preliminary single-step tests.
...
The stored attribution sidecars did not encode the integration budget in their filenames, and the $m{=}128$ extension ran concurrently with the variant stages on PegInsertionSide and PickSingleYCB.
...
A Month 5 reproduction run of the same checkpoint and seeds with real T5-XXL embeddings on a third machine produced 8/20 successes, with vision faithfulness medians inside the episode bootstrap intervals of the committed run (insertion 0.919 against 0.926 and deletion 0.264 against 0.248), and its records are committed.

\paragraph{Tasks.}
We evaluate on four ManiSkill3~\cite{tao2024maniskill3} manipulation tasks with a simulated Franka Panda 7-DoF arm.
...
PickSingleYCB-v1 grasps a YCB object.
...
The affected language numbers are the seed 42 RDT-1B language values reported with Table~\ref{tab:faithfulness}, the variant language correlations in Table~\ref{tab:sanity}, and the regeneration-run language distributions in Figure~\ref{fig:completeness}, so comparisons between them and main-pass language numbers span two baselines, while the vision and state baselines are unchanged.
...
The follow-up PickSingleYCB runs also reused the PickCube instruction embedding because the task's own embedding was unavailable, so no per-task language metrics are tabulated from them, the PickSingleYCB language distributions in Figure~\ref{fig:completeness} carry that proxy embedding, and the pooled variant language correlations include those proxy rows.
...
The Month 6 runs (the seed 142 and 242 RDT-1B faithfulness runs, the matched-population $\log\pi$ ablation, and the seed 142 displacement runs) ran on a second RunPod RTX PRO 6000 pod, which encoded its own T5-XXL instruction embeddings and two-token language baseline.
...
\caption{Completeness error distributions by task and modality over all policy calls of the committed regeneration run at $m{=}64$ (2 seeds $\times$ 15 episodes per task), with $m{=}128$ overlays on the PegInsertionSide and PickSingleYCB panels and the seed 42 RDT-1B scale-check rows overlaid on the PickCube panel. Vision and language mass concentrates at low error with tails extending past the 3\% line. State shows a longer right tail. The PickSingleYCB language histograms reflect the proxy PickCube instruction embedding recorded in Section~\ref{sec:setup}. All language distributions in this figure come from runs that used the zeroed language baseline recorded in Section~\ref{sec:setup}. Errors are clipped at 0.5 (50\%) for display, so the rightmost bin pools all larger errors.}
...
The pretrained-only policy itself compounds this, since on the fine-tuned 1B the language deletion AUC improves to passing and the insertion AUC improves but passes at only two of the three seeds (Table~\ref{tab:faithfulness}, bottom block), though that comparison spans the language-baseline deviation recorded in Section~\ref{sec:setup}.
...
The one seed-marginal case is language insertion, which passes at two of the three seeds while its pooled value sits just under its bar, so we read that pass as seed-sensitive rather than robust.
...
They are vision mis-focus, language mis-grounding, state misreading, delayed credit, and success-well-grounded.
...
The language AUC miss is accounted for by the token-grid coarseness and the 1B improvement in Section~\ref{sec:faithfulness}, and the state median miss by the per-dimension error accumulation noted with Table~\ref{tab:correctness}, which leaves three open items.
...
We therefore ran the full faithfulness protocol on RDT-1B with the authors' fine-tuned weights on PickCube at three evaluation seeds (20 episodes each, with 9, 1, and 5 successes, 1{,}199 pooled policy-call rows, with the signal filter inapplicable here as noted in Table~\ref{tab:faithfulness}).
...
The fine-tuned 1B operates at a roughly 3$\times$ smaller action norm, and at the matched top-5\% deletion fraction its relative displacement is at least as large as the 170M value at every solver-step count, with PickCube medians of 8.5\% against 5.1\% at $T{=}5$.
...
Under the sum-form target $-\frac{1}{2\sigma^2}\lVert a_t - \mu_t \rVert^2$, without the $1/D$ factor, every reported value scales by $D{=}512$, which would put the 170M vision magnitudes at roughly $-$0.7 to $-$2.0 nats (above the 0.5 bar) and the 1B value near $-$0.08 (below it).
...
The cross-scale displacement evidence is observational and comes from two tasks at two seeds at 1B scale and one seed at 170M, with 8 to 40 episodes per curve, and the faithfulness scale check covers one task at three seeds.
...
On the same matched population the $\log\pi$ deletion AUC misses its 0.40 bar, at 0.448 with a 95\% interval of 0.408 to 0.504, while both alternative targets pass it, so the main-pass deletion verdict does not carry over to this harder 0-success population.
...
Excluding the PickSingleYCB rows, which carry the embedding proxy recorded in Section~\ref{sec:setup}, moves the frozen language value from 0.862 to 0.843, moves every other frozen and cascade value by at most 0.024, and leaves the cascade language value unchanged.
...
The magnitude is a quadratic per-entry readout of a small action displacement, the smaller absolute displacement at 1B reflects its smaller action-norm regime while its relative displacement is at least as large, and the magnitude's relation to any fixed bar depends on the normalization convention.
...
State perturbation faithfulness is left to future work because the eight active state dimensions offer no graded deletion grid comparable to the 729 vision patches or the instruction token sequence, so state is the modality this evaluation covers least.
...
The failure taxonomy is assigned algorithmically from completeness signatures rather than by human annotation, and it merges spurious-background cases into vision mis-focus and ambiguous-instruction cases into language mis-grounding.
...
The RDT-1B stages additionally require the RDT authors' ManiSkill fine-tuned checkpoint from the robotics-diffusion-transformer maniskill-model repository on Hugging Face, without which the loader falls back to the Hugging Face base weights, applying the project LoRA only when a local checkpoint exists, and neither fallback reproduces the published 1B numbers.
The committed records are not a byte-level reproduction of every table.
The main evaluation pass and its faithfulness records, the original $m{=}128$ PickCube and StackCube subset, the standard sanity runs, the initial RDT-1B scale-check run, the per-step attribution sidecars, and the overlay images were produced on cloud GPUs that have since been decommissioned. None of them are part of the release.
...
Code, records, interval tables, and the analysis notebooks are available at \url{https://github.com/itscool2b/ig-work-2026}.
